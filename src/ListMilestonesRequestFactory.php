...
/**
 * Factory for creating requests to list repository milestones
 *
 ...
 */
class ListMilestonesRequestFactory
{
    ...
    {
        $url = sprintf(
            'https://api.github.com/repos/%s/%s/milestones?%s',
            $this->repo->owner,
            $this->repo->name,
            http_build_query(['state' => $this->state])
        );
