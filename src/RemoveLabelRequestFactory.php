...
/**
 * Factory for creating requests to remove a label from an issue/PR
 *
 ...
 */
class RemoveLabelRequestFactory
{
    ...
    /**
     * Create HTTP request to remove a label
     *
    ...
    {
        $url = sprintf(
            'https://api.github.com/repos/%s/%s/issues/%d/labels/%s',
            $this->repo->owner,
            $this->repo->name,
            $this->issueNumber,
            urlencode($this->labelName)
        );
